...
init(function()
    ...
    -- Introduced to use shortland form of names.
    if not _settings.aliases then _settings.aliases = {} end
    ...
    addEvents(function()
        ...
        EVENT_CURRENT_CAMPAIGN_CHANGED, EVENT_ASSIGNED_CAMPAIGN_CHANGED, EVENT_GUEST_CAMPAIGN_CHANGED,
    ...
    local function parseAlias(args)
        ...
        local buffer = string.match(args, "^[%d%a]+ ")
        local value = args:gsub(buffer, "", 1)
        local key = buffer:gsub(" ", "")
        return key, value
    end
    ...
    SLASH_COMMANDS["/goto"] = function(args)
        ...
        local aliasValue = _settings.aliases[args]
        if aliasValue then
            d(string.format("Alias '%s' used.", aliasValue))
    ...
    SLASH_COMMANDS["/agoto"] = function(args)
        ...
        if Utils.stringIsEmpty(key) or Utils.stringIsEmpty(key) then
            d(string.format("%s Failed to save alias for: %s", _prefix, args))
        else
            _settings.aliases[key] = value
            d(string.format("%s GOTO alias saved: %s -> %s", _prefix, key, value))
        end
    end
end)
