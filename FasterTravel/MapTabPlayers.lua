...
function MapTabPlayers:init(control)
	...
	self.Refresh = function()
		...
		local categories ={
			...
			{name = GetString(SI_MAP_INFO_PLAYERS_CATEGORY_GROUP).." ("..tostring(#group)..")", data = group, hidden= not _first and self:IsCategoryHidden(1)},
			{name = GetString(SI_MAP_INFO_PLAYERS_CATEGORY_FRIENDS).." ("..tostring(#friends)..")",data = friends, hidden= not _first and self:IsCategoryHidden(2)},
			{name = GetString(SI_MAP_INFO_PLAYERS_CATEGORY_ZONE),data=zones,hidden= not _first and self:IsCategoryHidden(3)}
		}
		...
		for i=1, gCount do 
			...
			table.insert(categories,{name=name.." ("..tostring(#data)..")",data=data,hidden=_first or self:IsCategoryHidden(i+3)})
		end
